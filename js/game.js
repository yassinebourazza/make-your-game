...
function BallMovement() {
    ...
    if (ballPlace.x + ballPlace.width+moveX >= containerPlace.x + (containerPlace.width)) {
        moveX = moveX <= 0 ? moveX : -moveX
    } else if (ballPlace.x+moveX <= containerPlace.x) {
        moveX = moveX >= 0 ? moveX : -moveX
    } else if (ballPlace.y+moveY <= containerPlace.y) {
        moveY = moveY >= 0 ? moveY : -moveY
    } else if (ballPlace.y + ballPlace.height + moveY >= containerPlace.y + (containerPlace.height)) {
        lives--
...
function ballTouchTheBreaker() {
    if (ballPlace.y + ballPlace.height + moveY >= breakerPlace.y && ballPlace.y <= breakerPlace.y 
...
function breakerMove() {
    if (moveRight && breakerPlace.x + breakerPlace.width  <= containerPlace.x + containerPlace.width) {
        BreakerX+= 7
        breaker.style.transform = `translateX(${BreakerX}px)`
    }
    if (moveLeft && breakerPlace.x >= containerPlace.x) {
        BreakerX-=7
        breaker.style.transform = `translateX(${BreakerX}px)`
    }
        ...
        if (ballPlace.y + ballPlace.height >= layers[layer].y) {
            for (let current of layers[layer].bricks) {
                ...
                if (ballPlace.x + (ballPlace.width / 2) >= br.x && ballPlace.x + (ballPlace.width / 2) <= br.x + br.width) {
                    ...
                    if ((ballPlace.x + moveX <= br.x && ballPlace.x + ballPlace.width + moveX >= br.x)
                        || (ballPlace.x + moveX <= br.x + br.width && ballPlace.x + ballPlace.width + moveX >= br.x + br.width)) {
                        current.remove()
                        score.innerHTML = Number.parseInt(score.innerHTML) + 50 
                        moveX = -moveX
                        bricks = document.getElementsByClassName('brick')
...
function resetGame() {
    ...
    currentY = 0
    BreakerX = 0
    breaker.style.transform = `translateX(0px)`
